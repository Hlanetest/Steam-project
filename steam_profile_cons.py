...
class handler():
    ...
    def get_game_id(self):
        ...
        url = "http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key={}&steamid={}=json".format(
            self.steamid, self.api_key)
        html_content = requests.get(url)
        json_response = json.loads(html_content.text)
        appIdList = {x["appid"]: {"playtime": x["playtime_forever"]}
                     for x in json_response["response"]["games"]}
        return appIdList

    def get_user_profile(self):
        '''Talks to the steam API, fetches user profiles. Stores array into username variable'''
        profile_url = "http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key={}&steamids={}".format(
            self.steamid, self.api_key,)
        user_content = requests.get(profile_url)
        user_response = json.loads(user_content.text)
        username = [x["personaname"]
                    for x in user_response["response"]["players"]]
        return username
